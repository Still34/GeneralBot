...
namespace GeneralBot.Extensions
{
    public static class UserExtensions
    {
        ...
        /// <returns></returns>
        public static Color GetHighestRoleColor(this SocketGuildUser user) => user.Roles
            .Where(x => !Equals(x.Color, Color.Default)).OrderByDescending(x => x.Position).Select(x => x.Color)
            .FirstOrDefault();
        ...
        public static string GetNicknameOrDefault(this IUser user) => (user as IGuildUser)?.Nickname ??
        ...
        public static string GetFullnameOrDefault(this IUser user) => user == null
            ? "InvalidUser#0000"
            : user.ToString();
        ...
        /// <returns></returns>
        public static string GetAvatarUrlOrDefault(this IUser user, ushort size = 128) =>
            user?.GetAvatarUrl(size: size) ?? "https://i.imgur.com/o5vbqOB.png";
    }
}